...
function setupPathAnimation(svgGroup, duration)
{
  ...
  allPaths.each(function(idx, p){
    ...
    var tween = TweenMax.fromTo(p,duration,
      {
        opacity: '1',
        strokeDasharray:String(pLen)+" "+String(pLen),
    ...
    if ($(p).attr('marker-end'))
    {
      var markerEnd = $(p).attr('marker-end');
      // nullify now
      $(p).attr('marker-end','');
...
function setupScrollScenes() {
  ...
  allAnnotations.each(function(idx, p){
    if (idx < allContents.length)
    {
      ...
      var scrollDuration = imgBlock.clientHeight-p.clientHeight;


      // console.log(idx+' img block ' + imgBlock.clientHeight + ' para '+p.clientHeight+' dur '+scrollDuration);
  ...
  // for SVG to animate:
  // - have inline SVG
  // - inside <svg> have animation steps split into groups using <g>
  ...
  {
    var svg = $('.scrollable-svg')[0];
    var animationGroups = $('.scrollable-svg>svg>g');
    var svgIdx = null;
    ...
    animationGroups.each(function(idx, g){
      ...
      {
        // g.style.opacity = "0";
        setupAnimationStep(annScrollScenes[idx-1], g);
      }
    });
...
// $( window ).resize(setupScrollScenes);
